class Sphere {
  ...
  render() {
    ...
    for (let i = 0; i <= stackCount; ++i) {
      ...
      for (let j = 0; j <= sectorCount; ++j) {
        ...
        // Calculate normal (normalized vector from center to surface)
  ...
  renderQuick() {
    ...
    const vertices = [];
    const texCoords = [];
    const normals = []; // Array for normals in quick render
    ...
    for (let lat = 0; lat <= segments; lat++) {
      ...
      for (let long = 0; long <= segments; long++) {
        ...
        // Normal is just the direction from center to surface point (normalized)
    ...
    // Draw the sphere
    if (vertices.length > 0) {
      // Single draw call using drawTriangle3DUVNormal
      const allVertices = [];
      const allUVs = [];
      const allNormals = [];
      ...
      // Use the new function that handles normals
